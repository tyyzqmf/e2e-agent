...
describe("JobService", () => {
  ...
  describe("Queue Operations", () => {
    ...
    test("dequeue returns job in FIFO order", () => {
      ...
      const jobId2 = freshService.submitJob(specFile);
  ...
  describe("deleteJob", () => {
    ...
    test("deletes completed job", () => {
      ...
      writeFileSync(specFile, "Test specification for delete completed job test");
  ...
  describe("deleteJobs (batch)", () => {
    ...
    test("handles partial failures", () => {
      ...
      writeFileSync(specFile, "Test specification for partial batch delete test");
  ...
  describe("getTestCases", () => {
    ...
    test("reads test cases from file", () => {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          test_cases: [
            { case_id: "TC-001", title: "Test 1", status: "Pass", priority: "P1" },
            { case_id: "TC-002", title: "Test 2", status: "Fail", priority: "P2" },
          ],
        })
      );
  ...
  describe("getLogPath", () => {
    ...
    test("returns path when log file exists", () => {
      ...
      const reportDir = join(tempDir, "reports", jobId);
      mkdirSync(reportDir, { recursive: true });
      writeFileSync(join(reportDir, "execution_stdout.log"), "Log content here");
  ...
  describe("getLogContent", () => {
    ...
    test("returns full content when tail is 0", () => {
      ...
      const reportDir = join(tempDir, "reports", jobId);
      mkdirSync(reportDir, { recursive: true });
      writeFileSync(join(reportDir, "execution_stdout.log"), "Line 1\nLine 2\nLine 3\nLine 4\nLine 5");

...
describe("E2E CLI", () => {
  ...
  describe("check command", () => {
    test("checks environment requirements", async () => {
      ...
      expect(output).toContain("Python 3.7+:");
  ...
  describe("job commands", () => {
    ...
    test("job delete with non-existent id shows error", async () => {
      const result = await $`bun run ${CLI_PATH} job delete non-existent-job-id`.quiet().nothrow();
  ...
  describe("unknown command", () => {
    test("shows error for unknown command", async () => {
      const result = await $`bun run ${CLI_PATH} unknown-command`.quiet().nothrow();
...
describe("E2E CLI Environment Check", () => {
  test("checkPython returns boolean", async () => {
  ...
  test("getPythonVersion returns version string", async () => {

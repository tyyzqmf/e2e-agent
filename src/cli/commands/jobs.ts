  ...
  try {
    ...
    if (!logPath) {
      ...
      if (job.status === "queued") {
        printInfo("Job has not started yet. Logs will be available after execution begins.");
      }
      process.exit(1);
    }
 ...
 */
function parseLogOptions(args: string[]): { jobId: string; tail?: number; follow?: boolean } {
  const options: { jobId: string; tail?: number; follow?: boolean } = { jobId: "" };
 ...
 * Handle job command
 */
export async function handleJobCommand(action: string, args: string[]): Promise<void> {
  switch (action) {
    case "submit":
    ...
    case "log": {
      ...
      await getJobLog(options.jobId, { tail: options.tail, follow: options.follow });
      break;
    }

    default:
      printError(`Unknown job action: ${action}`);
      console.log("Usage: e2e job <submit|cancel|status|list|delete|log> [args]");
      process.exit(1);
  }
}

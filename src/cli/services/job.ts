...
import { PROJECT_ROOT, DATA_DIR, colors } from "../utils.ts";
...
export class JobService {
  ...
  submitJob(specFile: string): string | null {
    ...
    if (!existsSync(specPath)) {
      console.error(`${colors.red}Error:${colors.reset} File not found: ${specFile}`);
      return null;
    }
    ...
    try {
      ...
      const transaction = this.db.transaction(() => {
        ...
        this.db
          .prepare(`INSERT INTO queue (job_id, created_at) VALUES ($jobId, $createdAt)`)
          .run({
            $jobId: jobId,
            $createdAt: createdAt,
          });
      ...
      console.error(`${colors.red}Error:${colors.reset} Failed to submit job: ${error}`);
      return null;
    }
  }
  ...
  cancelJob(jobId: string): { success: boolean; message: string } {
    ...
    if (job.status === "queued") {
      ...
      const transaction = this.db.transaction(() => {
        this.db.prepare(`DELETE FROM queue WHERE job_id = $jobId`).run({ $jobId: jobId });
        this.db
          .prepare(
    ...
    if (job.status === "running") {
      this.db.prepare(`UPDATE jobs SET stop_requested = 1 WHERE job_id = $jobId`).run({
        $jobId: jobId,
      });
      return { success: true, message: "Stop request sent. Job will terminate at next check." };
    }
  ...
  getCostStatistics(jobId: string): CostStatistics | null {
    const paths = [
      join(this.reportsDir, jobId, "generations", jobId, "cost_statistics.json"),
  ...
  dequeue(): string | null {
    const stmt = this.db.prepare(`SELECT job_id FROM queue ORDER BY created_at ASC LIMIT 1`);
    const row = stmt.get() as { job_id: string } | null;

    if (!row) return null;

    this.db.prepare(`DELETE FROM queue WHERE job_id = $jobId`).run({ $jobId: row.job_id });
    return row.job_id;
  }
  ...
  updateJobStatus(jobId: string, status: JobStatus, errorMessage?: string): void {
    const isCompleted = ["completed", "failed", "stopped", "cancelled"].includes(status);
    const now = new Date().toISOString();

    if (status === "running") {
      this.db.prepare(`UPDATE jobs SET status = 'running', started_at = $now WHERE job_id = $jobId`).run({
        $jobId: jobId,
        $now: now,
      });
    } else if (isCompleted) {
      ...
      this.db.prepare(`UPDATE jobs SET status = $status WHERE job_id = $jobId`).run({
        $jobId: jobId,
        $status: status,
      });
    }
  ...
  setProcessPid(jobId: string, pid: number): void {
    this.db.prepare(`UPDATE jobs SET process_pid = $pid WHERE job_id = $jobId`).run({
      $jobId: jobId,
      $pid: pid,
    });
  }
  ...
  recoverOrphanJobs(): number {
    const stmt = this.db.prepare(`SELECT job_id, process_pid FROM jobs WHERE status = 'running'`);
    const runningJobs = stmt.all() as { job_id: string; process_pid: number | null }[];
    ...
    for (const job of runningJobs) {
      ...
      if (!processAlive) {
        const transaction = this.db.transaction(() => {
          ...
          this.db
            .prepare(`INSERT OR REPLACE INTO queue (job_id, created_at) VALUES ($jobId, $now)`)
  ...
  private cleanupJobFiles(jobId: string): void {
    ...
    if (existsSync(reportDir)) {
      try {
        ...
        console.error(`Failed to clean up report directory: ${reportDir}`, error);
      }
    }
  }

  deleteJobs(jobIds: string[]): { success: boolean; deleted: string[]; failed: { jobId: string; reason: string }[] } {
...
export function printJobStatus(job: Job, cost: CostStatistics | null, testCases: TestCaseSummary[] | null): void {
  console.log("\nJob Details:");

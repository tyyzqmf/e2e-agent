...
import { JobService, type Job, type JobStatus } from "./job.ts";
import { PROJECT_ROOT, DATA_DIR, LOGS_DIR, colors, writePid, removePidFile, EXECUTOR_PID_FILE } from "../utils.ts";
...
export class TestExecutor {
  ...
  private async executeTest(jobId: string): Promise<void> {
    try {
      ...
      if (exitCode === 0) {
        ...
        const errorMsg = this.getMeaningfulErrorMessage(jobId, stderr, exitCode);
        this.jobService.updateJobStatus(jobId, "failed", errorMsg);
        log("WARN", `Job ${jobId} failed: ${errorMsg}`);
      } else {
        const errorMsg = this.getMeaningfulErrorMessage(jobId, stderr, exitCode);
        this.jobService.updateJobStatus(jobId, "failed", errorMsg);
        log("ERROR", `Job ${jobId} failed with exit code ${exitCode}: ${errorMsg}`);
      }
    } catch (error) {
      log("ERROR", `Failed to execute job ${jobId}: ${error}`);
      this.jobService.updateJobStatus(jobId, "failed", `Execution error: ${error}`);
    } finally {
  ...
  private startStopCheck(jobId: string): void {
    const check = () => {
      if (!this.running || !this.currentProcess || this.currentJobId !== jobId) return;
  ...
  private getMeaningfulErrorMessage(jobId: string, stderr: string, exitCode: number): string {
    // Try to read from test_cases.json
    try {
      const testCasesFile = join(this.config.reportsDir, jobId, "test_cases.json");

      if (existsSync(testCasesFile)) {
        const testCases = JSON.parse(readFileSync(testCasesFile, "utf-8"));
        const cases = Array.isArray(testCases) ? testCases : testCases.test_cases ?? [];

        const stats: Record<string, number> = { Pass: 0, Fail: 0, Blocked: 0, "Not Run": 0 };
        ...
        if (failedCases.length > 0) {
          return `Tests Failed (${stats.Fail}/${total}). Passed: ${stats.Pass}, Blocked: ${stats.Blocked}. First failure: ${failedCases[0]}`.slice(0, 500);
        }
  ...
  private prepareWorkspace(job: Job): string {
    ...
    if (job.envConfig && Object.keys(job.envConfig).length > 0 && !testSpecContent.includes("## Environment Configuration")) {
      let envSection = "\n\n## Environment Configuration\n\n";

      // Application access
      const appUrl = job.envConfig.PORTAL_URL ?? job.envConfig.APPLICATION_URL ?? "";
      if (appUrl) {
      ...
      // Other configs
      const skipKeys = ["PORTAL_URL", "APPLICATION_URL", "API_BASE_URL", "USER", "USERNAME", "PASSWORD"];
      const otherConfigs = Object.entries(job.envConfig).filter(([k]) => !skipKeys.includes(k));

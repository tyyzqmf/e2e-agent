...
export async function checkChrome(): Promise<boolean> {
	...
	try {
		const result =
			await $`test -x "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"`
				.quiet()
				.nothrow();
		if (result.exitCode === 0) {
			return true;
	...
	const commands = [
		{
			...
			extract: (s: string) => s.split(" ").pop() || "Unknown",
		},
		{
		...
		},
		{
			...
			extract: (s: string) => s.split(" ")[1] || "Unknown",
		},
		{
 ...
 */
export async function checkRequirementsOrExit(
	verbose: boolean = true,
): Promise<void> {

...
async function loadJobStatus() {
  ...
  try {
    ...
    if (job.started_at) {
      ...
      document.getElementById('startedAt').textContent = formatDate(job.started_at);
    } else {
    ...
    if (job.completed_at) {
      ...
      document.getElementById('completedAt').textContent = formatDate(job.completed_at);
    } else {
...
function updateCost(cost) {
  ...
  document.getElementById('inputTokens').textContent = formatNumber(cost.input_tokens || 0);
  document.getElementById('outputTokens').textContent = formatNumber(cost.output_tokens || 0);
  document.getElementById('totalCost').textContent = `$${(cost.total_cost || 0).toFixed(4)}`;
}

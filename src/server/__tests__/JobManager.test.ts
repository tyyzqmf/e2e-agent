...
describe("JobManager", () => {
  ...
  describe("createJob", () => {
    ...
    it("should throw error for empty test spec", () => {
      expect(() => jobManager.createJob("")).toThrow("test_spec cannot be empty");
    });
    ...
    it("should throw error for test spec exceeding size limit", () => {
      ...
      expect(() => jobManager.createJob(largeSpec)).toThrow("test_spec exceeds size limit");
    });
  });

  describe("getJob", () => {
    it("should retrieve existing job", () => {
      const original = jobManager.createJob("Test spec with at least 10 characters");
  ...
  describe("listJobs", () => {
    ...
    it("should respect limit parameter", () => {
      for (let i = 0; i < 10; i++) {
        jobManager.createJob(`Spec ${i} with enough characters to pass validation`);
      }
  ...
  describe("deleteJob", () => {
    it("should delete a queued job", () => {
      const job = jobManager.createJob("Test spec to delete with enough characters");
    ...
    it("should delete a completed job", () => {
      const job = jobManager.createJob("Test spec for completed with enough characters");
      jobManager.updateJobStatus(job.jobId, "completed");
    ...
    it("should delete a failed job", () => {
      const job = jobManager.createJob("Test spec for failed with enough characters");
    ...
    it("should not delete a running job", () => {
      const job = jobManager.createJob("Test spec for running with enough characters");
      jobManager.dequeue();
      jobManager.markJobStarted(job.jobId);
    ...
    it("should clean up job files when deleting", () => {
      const job = jobManager.createJob("Test spec for file cleanup with enough characters");
  ...
  describe("deleteJobs", () => {
    it("should delete multiple jobs", () => {
      const job1 = jobManager.createJob("Spec 1 for batch delete with enough chars");
      const job2 = jobManager.createJob("Spec 2 for batch delete with enough chars");
      const job3 = jobManager.createJob("Spec 3 for batch delete with enough chars");

      const result = jobManager.deleteJobs([job1.jobId, job2.jobId, job3.jobId]);
    ...
    it("should handle partial failures in batch delete", () => {
      const job1 = jobManager.createJob("Spec 1 for partial batch with enough chars");
      const job2 = jobManager.createJob("Spec 2 for partial batch with enough chars");
      ...
      const result = jobManager.deleteJobs([job1.jobId, job2.jobId, "non-existent"]);

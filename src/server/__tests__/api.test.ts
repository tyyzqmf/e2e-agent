...
import { buildApiRoutes, buildHealthRoute, handleOptions } from "../routes/api.ts";
...
describe("API Routes", () => {
  ...
  describe("POST /api/jobs", () => {
    it("should create a new job", async () => {
      const req = new Request("http://localhost/api/jobs", {
        ...
        body: JSON.stringify({
          test_spec: "Test specification content with at least 10 characters for validation",
        }),
      });
    ...
    it("should create job with env_config", async () => {
      const req = new Request("http://localhost/api/jobs", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          test_spec: "Test specification with at least 10 characters for validation",
  ...
  describe("GET /api/jobs", () => {
    it("should list all jobs", async () => {
      ...
      for (let i = 0; i < 3; i++) {
        services.jobManager.createJob(`Spec ${i} with enough characters for validation`);
      }
  ...
  describe("GET /api/jobs/:id", () => {
    it("should get job status", async () => {
      const job = services.jobManager.createJob("Test spec for status with enough characters");
  ...
  describe("POST /api/jobs/:id/stop", () => {
    it("should stop a queued job", async () => {
      const job = services.jobManager.createJob("Test spec to stop with enough characters");
  ...
  describe("Health Check", () => {
    it("should return health status", async () => {
      const executor = { getCurrentJobId: () => null };
      const healthRoutes = buildHealthRoute(services, executor);
  ...
  describe("GET /api/jobs/:id/report", () => {
    ...
    it("should return error when job is not completed", async () => {
      const job = services.jobManager.createJob("Test spec for report with enough characters");
    ...
    it("should return HTML report when job is completed and report exists", async () => {
      const job = services.jobManager.createJob("Test spec for completed report with enough characters");
      // Manually mark job as completed
      services.jobManager.updateJobStatus(job.jobId, "completed");

      // Create the report file
      const reportDir = join(tempDir, "reports", job.jobId, "test-reports", "20250101");
      mkdirSync(reportDir, { recursive: true });
      writeFileSync(join(reportDir, "Test_Report_Viewer.html"), "<html><body>Test Report</body></html>");
    ...
    it("should return 404 when job is completed but report file is missing", async () => {
      const job = services.jobManager.createJob("Test spec for missing report with enough characters");
      services.jobManager.updateJobStatus(job.jobId, "completed");
  ...
  describe("GET /api/jobs/:id/download", () => {
    ...
    it("should return error when job is not completed", async () => {
      const job = services.jobManager.createJob("Test spec for download with enough characters");

      const req = new Request(`http://localhost/api/jobs/${job.jobId}/download`);
      (req as any).params = { id: job.jobId };
  ...
  describe("POST /api/jobs/:id/stop - Error Cases", () => {
    ...
    it("should return error when trying to stop already completed job", async () => {
      const job = services.jobManager.createJob("Test spec to stop completed job with enough chars");
      services.jobManager.updateJobStatus(job.jobId, "completed");
  ...
  describe("DELETE /api/jobs/:id", () => {
    it("should delete a queued job", async () => {
      const job = services.jobManager.createJob("Test spec to delete with enough characters");
    ...
    it("should delete a completed job", async () => {
      const job = services.jobManager.createJob("Test spec for completed delete with enough chars");
      services.jobManager.updateJobStatus(job.jobId, "completed");
    ...
    it("should not delete a running job", async () => {
      const job = services.jobManager.createJob("Test spec for running delete with enough chars");
  ...
  describe("DELETE /api/jobs (batch)", () => {
    it("should delete multiple jobs", async () => {
      const job1 = services.jobManager.createJob("Spec 1 for batch delete API with enough chars");
      const job2 = services.jobManager.createJob("Spec 2 for batch delete API with enough chars");
    ...
    it("should handle partial failures", async () => {
      const job1 = services.jobManager.createJob("Spec 1 for partial API batch with enough chars");
      const job2 = services.jobManager.createJob("Spec 2 for partial API batch with enough chars");
      services.jobManager.dequeue();
      services.jobManager.dequeue();
  ...
  describe("GET /api/jobs/:id/logs", () => {
    ...
    it("should return has_log=false when job exists but no log", async () => {
      const job = services.jobManager.createJob("Test spec for logs with enough characters");
    ...
    it("should return log content when log exists", async () => {
      const job = services.jobManager.createJob("Test spec for log content with enough characters");

      // Create the log file
      const logDir = join(tempDir, "reports", job.jobId);
      mkdirSync(logDir, { recursive: true });
      writeFileSync(join(logDir, "execution_stdout.log"), "Line 1\nLine 2\nLine 3\n");
    ...
    it("should respect tail parameter", async () => {
      const job = services.jobManager.createJob("Test spec for tail param with enough characters");
      ...
      const req = new Request(`http://localhost/api/jobs/${job.jobId}/logs?tail=2`);
      (req as any).params = { id: job.jobId };
    ...
    it("should return text format when requested", async () => {
      const job = services.jobManager.createJob("Test spec for text format with enough characters");
      ...
      const req = new Request(`http://localhost/api/jobs/${job.jobId}/logs?format=text`);
      (req as any).params = { id: job.jobId };
    ...
    it("should show appropriate message for queued job without log", async () => {
      const job = services.jobManager.createJob("Test spec for queued job logs with enough chars");
  ...
  describe("GET /api/jobs/:id/download - Success Path", () => {
    it("should create and return ZIP file when report exists", async () => {
      const job = services.jobManager.createJob("Test spec for ZIP download with enough characters");
      services.jobManager.updateJobStatus(job.jobId, "completed");

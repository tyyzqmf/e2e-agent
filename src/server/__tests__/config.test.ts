...
import { config, validateConfig, printConfig } from "../config.ts";
...
describe("Config", () => {
  describe("config object", () => {
    ...
    it("should have all validation config properties", () => {
      ...
      expect(config.TEST_SPEC_MAX_LENGTH).toBeGreaterThan(config.TEST_SPEC_MIN_LENGTH);
    });
  ...
  describe("validateConfig", () => {
    ...
    it("should validate EXECUTOR_POLL_INTERVAL_MS minimum", () => {
      const errors = validateConfig();
      const pollError = errors.find((e) => e.includes("EXECUTOR_POLL_INTERVAL_MS"));
  ...
  describe("printConfig", () => {
    ...
    it("should print header lines", () => {
      ...
      const hasSeparator = calls.some((line) => line && line.includes("="));
    ...
    it("should print runtime info", () => {
      ...
      const calls = consoleSpy.mock.calls.map((c) => c[0]);
      const hasRuntime = calls.some((line) => line && line.includes("Runtime"));
    ...
    it("should print environment info", () => {
      ...
      const calls = consoleSpy.mock.calls.map((c) => c[0]);
      const hasEnv = calls.some((line) => line && line.includes("Environment"));
    ...
    it("should print server info", () => {
      ...
      const calls = consoleSpy.mock.calls.map((c) => c[0]);
      const hasServer = calls.some((line) => line && line.includes("Server"));
    ...
    it("should print data directory", () => {
      ...
      const calls = consoleSpy.mock.calls.map((c) => c[0]);
      const hasDataDir = calls.some((line) => line && line.includes("Data Dir"));
    ...
    it("should print database path", () => {
      ...
      const calls = consoleSpy.mock.calls.map((c) => c[0]);
      const hasDatabase = calls.some((line) => line && line.includes("Database"));
    ...
    it("should print log level", () => {
      ...
      const calls = consoleSpy.mock.calls.map((c) => c[0]);
      const hasLogLevel = calls.some((line) => line && line.includes("Log Level"));

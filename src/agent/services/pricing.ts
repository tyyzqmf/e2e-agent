...
export class PricingCalculator {
  ...
  getRates(model: string): PricingRates {
    ...
    if (cachedPrices) {
      ...
      if (rates) {
        console.log(
          `[Pricing] WARNING: Using expired cache for ${model}`
        );
        return rates;
      }
    }
  ...
  async getRatesAsync(model: string): Promise<PricingRates> {
    ...
    console.log(`[Pricing] Cache is stale or missing, fetching latest prices...`);
  ...
  isCacheValid(): boolean {
    ...
    try {
      ...
      const content = require("fs").readFileSync(this.cacheFile, "utf-8");
  ...
  private extractModelRates(
    ...
    try {
      return {
        ...
        cacheReadRate:
          (modelInfo.cache_read_input_token_cost ?? 0) * 1_000_000,
      };
    } catch {
      return null;
    }

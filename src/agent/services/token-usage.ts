...
export async function updateHtmlReportCostStatistics(
	...
	if (!existsSync(statsFile)) {
		console.log(
			"[Cost Stats] No usage_statistics.json found, skipping HTML update",
		);
		return false;
	}
	...
	try {
		const content = require("node:fs").readFileSync(statsFile, "utf-8");
	...
	if (!updated) {
		console.log("[Cost Stats] No cost placeholders found in HTML report");
		return false;
	}

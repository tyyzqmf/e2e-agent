...
import { getCompletionRate, getPassRate, getCompletedCount } from "../types/test-case.ts";
 ...
 */
export async function loadTestCases(testCasesFile: string): Promise<TestCase[]> {
 ...
 */
export async function countTestCases(projectDir: string): Promise<TestCaseStats> {
 ...
 */
export async function printTestProgressSummary(projectDir: string): Promise<void> {
  ...
  console.log(`  └─ Passed: ${stats.passed} (${passRate.toFixed(1)}% of completed)`);

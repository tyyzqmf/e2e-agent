...
import { TokenUsageTracker, CostReportGenerator, updateHtmlReportCostStatistics } from "../services/token-usage.ts";
...
describe("Prompts Service", () => {
  ...
  test("validateDestName rejects absolute paths", () => {
    ...
    expect(() => validateDestName("\\windows\\system32")).toThrow("Absolute paths");
  });
...
describe("Progress Service", () => {
  ...
  test("countDefects counts defect files correctly", async () => {
    const defectsDir = join(testDir, "test-reports", "2025-01-01", "defect-reports");
...
describe("Pricing Service", () => {
  test("PricingCalculator.getFallbackRates returns rates for known models", () => {
    ...
    const sonnetRates = calculator.getFallbackRates("claude-sonnet-4-5-20250929");
  ...
  test("PricingCalculator.getFallbackRates handles AWS Bedrock model IDs", () => {
    ...
    const rates = calculator.getFallbackRates("us.anthropic.claude-sonnet-4-5-20250929-v1:0");
...
describe("Token Usage Service", () => {
  ...
  test("TokenUsageTracker.displaySessionStats outputs formatted stats", () => {
    ...
    expect(consoleLogs.some(log => log.includes("SESSION STATISTICS"))).toBe(true);
    ...
    expect(consoleLogs.some(log => log.includes("Project Totals"))).toBe(true);
  });
...
describe("Prompts Service - Extended", () => {
  const tempProjectDir = join(tmpdir(), `e2e-prompts-test-${Date.now()}`);
  ...
  test("validateProjectDirectory rejects paths outside expected boundaries", () => {
    expect(() => validateProjectDirectory("/tmp/outside-project")).toThrow("Invalid project directory");
  });
  ...
  test("copyToProject copies file to destination", () => {
    ...
    expect(readFileSync(join(generationsDir, "copied.txt"), "utf-8")).toBe("test content");
  });
  ...
  test("copyToProject validates destName for path traversal", () => {
    const generationsDir = join(process.cwd(), "generations", "test-prompts");
    expect(() => copyToProject(generationsDir, "/tmp/source.txt", "../escape.txt", false)).toThrow("Path traversal");
  });
  ...
  test("copyToProject copies directory recursively", () => {
    ...
    expect(existsSync(join(generationsDir, "copied-dir", "file1.txt"))).toBe(true);
    expect(existsSync(join(generationsDir, "copied-dir", "subdir", "file2.txt"))).toBe(true);
  });

  test("copyToProject throws on copy error", () => {
    const generationsDir = join(process.cwd(), "generations", "test-prompts");
    // Try to copy a non-existent source
    expect(() => copyToProject(generationsDir, "/nonexistent/source.txt", "dest.txt", false)).toThrow();
  });
});
...
describe("Progress Service - Extended", () => {
  ...
  test("loadTestCases throws for non-existent file", async () => {
    await expect(loadTestCases(join(progressTestDir, "nonexistent.json"))).rejects.toThrow("File not found");
  });
  ...
  test("printTestProgressSummary outputs progress when tests exist", async () => {
    ...
    expect(logs.some(log => log.includes("Test Execution Progress"))).toBe(true);
  ...
  test("printTestProgressSummary shows defect count", async () => {
    ...
    const defectsDir = join(progressTestDir, "test-reports", "2025-01-01", "defect-reports");
    mkdirSync(defectsDir, { recursive: true });
    ...
    expect(logs.some(log => log.includes("Total Defects Reported: 1"))).toBe(true);
  });
...
describe("Pricing Service - Extended", () => {
  ...
  test("PricingCalculator.getFallbackRates handles anthropic. prefix", () => {
    ...
    const rates = calculator.getFallbackRates("anthropic.claude-sonnet-4-5-20250929");
    expect(rates.inputRate).toBe(3.0);
  });
});
...
describe("Prompts Service - Setup Functions", () => {
  const setupTestDir = join(process.cwd(), "generations", `setup-test-${Date.now()}`);
...
describe("Token Usage Service - Extended", () => {
  ...
  test("updateHtmlReportCostStatistics handles HTML without placeholders", async () => {
    ...
    const reportDir = join(tokenTestDir, "test-reports", "2025-01-01");
    mkdirSync(reportDir, { recursive: true });
    writeFileSync(join(reportDir, "Test_Report_Viewer.html"), "<html><body>No placeholders</body></html>", "utf-8");
  ...
  test("updateHtmlReportCostStatistics updates HTML with placeholders", async () => {
    ...
    const updatedHtml = readFileSync(join(reportDir, "Test_Report_Viewer.html"), "utf-8");
  ...
  test("updateHtmlReportCostStatistics handles invalid stats JSON", async () => {
    ...
    writeFileSync(join(tokenTestDir, "usage_statistics.json"), "invalid json", "utf-8");
  ...
  test("TokenUsageTracker handles corrupted stats file", () => {
    ...
    expect(summary.totalSessions).toBe(0);
    expect(logs.some(log => log.includes("Could not load statistics"))).toBe(true);
  });

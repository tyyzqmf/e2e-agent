...
describe("Tools Configuration", () => {
  ...
  test("CHROME_DEVTOOLS_TOOLS contains expected tools", () => {
    expect(CHROME_DEVTOOLS_TOOLS).toContain("mcp__chrome-devtools__navigate_page");
    expect(CHROME_DEVTOOLS_TOOLS).toContain("mcp__chrome-devtools__take_screenshot");
...
describe("MCP Servers Configuration", () => {
  ...
  test("getChromeDevtoolsConfig returns correct default config", () => {
    ...
    expect(config.args.some(arg => arg.includes("--executablePath"))).toBe(true);
  });
  ...
  test("createCustomMcpServer creates correct config", () => {
    const server = createCustomMcpServer("node", ["server.js"], { API_KEY: "test" });
...
describe("Hooks Configuration", () => {
  test("constants are defined correctly", () => {
    ...
    expect(CONTEXT_MANAGEMENT_PROMPT).toContain("Context Management Guidelines");
  });

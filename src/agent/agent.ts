...
import { createClient, type ClaudeClient, type AgentMessage } from "./client.ts";
import {
  SessionStatus,
  type SessionResult,
  type UsageData,
...
export async function runAutonomousTestingAgent(
  ...
  while (true) {
    ...
    if (status === SessionStatus.CONTINUE) {
      ...
      if (stats.notRun === 0 && stats.total > 0) {
        ...
        console.log(
          "  Final reports should have been generated by the agent."
        );
        console.log("=".repeat(70));
        break;
      }
  ...
  try {
    ...
    if (existsSync(testReportsDir)) {
      ...
      const costReportPath = join(projectDir, "cost_statistics.md");
      require("fs").writeFileSync(costReportPath, costReport, "utf-8");
      console.log(`\n[Cost Report] Saved to: ${costReportPath}`);
    }
  } catch (error) {
